...
//    BENCHMARK_Q((res_list = server_->search_parallel_light(message_to_request(mes),std::thread::hardware_concurrency())),res_list.size(), PRINT_BENCH_SEARCH_PAR_NORPC)
    ...
    if (mes->add_count() <= 10) {
        BENCHMARK_Q((res_list = server_->search_parallel_light(message_to_request(mes),std::thread::hardware_concurrency())), mes->derivation_key(), res_list.size(), PRINT_BENCH_SEARCH_PAR_NORPC)
...
//        BENCHMARK_Q((res_list = server_->search_parallel_light(message_to_request(mes),std::thread::hardware_concurrency())),res_list.size(), PRINT_BENCH_SEARCH_PAR_NORPC)
        BENCHMARK_Q((res_list = server_->search_parallel(message_to_request(mes),2)), mes->derivation_key(),  res_list.size(), PRINT_BENCH_SEARCH_PAR_NORPC);
