...
#define BENCHMARK_Q(block, keyword, quotient, comment_f) \
{\
auto begin = std::chrono::high_resolution_clock::now(); \
block; \
auto end = std::chrono::high_resolution_clock::now(); \
std::chrono::duration<double, std::milli> time_ms = end - begin; \
{sse::logger::log_benchmark() << comment_f(keyword, time_ms.count(), quotient) << std::endl;} \
}

#else
#define BENCHMARK_SIMPLE(comment, block) block;
#define BENCHMARK_Q(block, keyword, quotient, comment_f) block;
#endif
